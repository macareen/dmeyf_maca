...
karch_dataset    <- "./datasets/dataset_epic_simple_v001.csv.gz"   #este dataset se genero en el script 812_dataset_epic.r
...
ktrain_mes_hasta    <- 201801  #Obviamente, solo puedo entrenar hasta 202011
ktrain_mes_desde    <- 202012

ktrain_meses_malos  <- c()  #meses que quiero excluir del entrenamiento

kgen_mes_hasta    <- 201801  #Obviamente, solo puedo entrenar hasta 202011
kgen_mes_desde    <- 202012
...
hs <- makeParamSet( 
        makeNumericParam("learning_rate",      lower= 0.01  , upper=    0.1), #(modifique el 0,1 a 0,05 en upper)
        makeNumericParam("feature_fraction",   lower= 0.1   , upper=    1.0), #(modifique el 0,2 a 0,1 en lower)
        ...
        makeNumericParam("lambda_l1",          lower= 0     , upper=     40), # 0-19
        makeNumericParam("lambda_l2",          lower= 0     , upper=    250), # 30-150
        makeIntegerParam("max_depth",          lower= 1L    , upper=  8000L),
        makeIntegerParam("max_bin",            lower= 10L   , upper=    45L), 
        # AGREGADOS por leer articulo
        makeNumericParam("bagging_fraction",   lower= 0.7   , upper=      1),
        makeIntegerParam("bagging_freq",       lower= 0     , upper=    100) 
)


campos_malos  <- c("ccajas_transacciones", "internet")   #aqui se deben cargar todos los campos culpables del Data Drifting
...
ksemilla_azar  <- 107077 #Aqui poner la propia semilla
#------------------------------------------------------------------------------
